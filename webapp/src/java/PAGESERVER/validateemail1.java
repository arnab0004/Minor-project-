 ...
 * @author POUSHALI
 */
public class validateemail1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet validateemail1</title>");            
            out.println("</head>");
            out.println("<body>");
           out.println("");
        ...
        protected PasswordAuthentication getPasswordAuthentication()
        {
            return new PasswordAuthentication(username,password);
                ...
                try {
                    ...
                    vbody += "http://localhost:8080/webapp/webpack/verifyques.jsp?pemail="+vemail;
                    message.setText(vbody);
                    Transport.send(message);
                    response.sendRedirect("http://localhost:8080/webapp/STATPAGES/forgotclose.html");
                } catch (MessagingException ex) {
                    out.println("<h2 style='color:red>+ex.getMessage()+</h2>");
                    Logger.getLogger(validateemail1.class.getName()).log(Level.SEVERE, null, ex);
                }

}
else
{
    response.sendRedirect("http://localhost:8080/webapp/STATPAGES/wrongemail.html");
}
ost.close();
oconn.close();

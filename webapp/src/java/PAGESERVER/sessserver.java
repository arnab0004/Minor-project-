 ...
 * @author POUSHALI
 */
public class sessserver extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet sessserver</title>");            
            out.println("</head>");
            out.println("<body>");
        ...
        ost=(OraclePreparedStatement)oconn.prepareStatement("SELECT * FROM USERS1 where email=? and password=?");
        ...
        if(ors.next())
        {
            ...
            Session session=Session.getInstance(props,new javax.mail.Authenticator()
            {
                ...
                try {
                    ...
                    response.sendRedirect("http://localhost:8080/webapp/webpack/verifyOTP.jsp");
                    
                } catch (MessagingException ex) {
                    out.println("<h2 style='color:red'>"+ex.getMessage()+"</h2>");
                
                    Logger.getLogger(sessserver.class.getName()).log(Level.SEVERE, null, ex);
                }    
        }
        else
        {
